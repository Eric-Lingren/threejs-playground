...
import test2 from '../assets/test.jpeg'
...
const Three = () => {
    ...
    useEffect(() => {
        ...
        scene.background = new THREE.Color(0x156477)
        ...
        // const helper = new THREE.PlaneHelper( plane, 1, 0x000000 );
        // scene.add( helper );
        ...
        // var texture2 = new THREE.TextureLoader().load(test2)
        ...
        for(let i = 0; i < 200; i++){
            var item = new THREE.Mesh( geometry2, material2)
            // item.position.x = Math.random() * 1;
            item.position.x = Math.random() * 800 - 400;
            // item.position.y = Math.random() * 800 - 400;
            item.position.y = Math.random() * 1 -10;
            item.position.z = Math.random() * 800 - 400;
            scene.add( item )
        }
        ...
        // controls.maxPolarAngel = Math.PI / 1.5;
        controls.update();

        var planeGeo = new THREE.PlaneBufferGeometry(500, 500)
        planeGeo.rotateX(4.7)
        var texture1 = new THREE.TextureLoader().load(test)
        var planeMaterial = new THREE.MeshBasicMaterial({ map: texture1, flatShading: true})
